...
- 新增“关于 / 版本”窗口，显示本地当前版本与公开 GitHub Release 的云端最新版本。
- 可以在软件内查看 Release 正文、发布日期，或直接打开对应 GitHub 更新页。
- 增加 GitHub 项目地址和自愿赞助入口；明确软件持续免费，不因赞助改变功能或更新权限。
- GitHub API 暂时不可用时，版本页面仍会尝试读取公开备用清单；失败只提示查询失败，不影响本地点名和 Excel。
- 暂不写入未确定的独立官网地址，当前以 GitHub 项目仓库作为官方发布地址。
- 发布脚本支持 `HX-HAM-X.Y.Z` 测试标签；测试版与稳定自动更新清单隔离。
- 测试构建会把完整的 `HX-HAM-X.Y.Z` 写入单文件 EXE；测试版只查询同名 GitHub
  Release，不会把稳定版 `v0.9.4` 错当成云端版本。

## HX-HAM-0.0.2 (2026-08-24，测试版)

- 修复测试版下载后仍显示 `0.9.4` 的问题，EXE 内置版本改为 `HX-HAM-0.0.2`。
- 更新检查改为查询 `HX-HAM-0.0.2` 精确 tag，关于页面显示测试 Release 的完整标签和说明。
- 测试版仍使用真实 Release 附件和 SHA256 校验；不会改变稳定版用户的自动更新通道。
...
- 当前测试加载器共发现 248 项；Python 3.13 与 3.14 均已用单进程完整执行 248 项并全部通过。
- Ruff 静态检查通过。
- `compileall` 通过。
